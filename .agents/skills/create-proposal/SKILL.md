...
- 单页或单能力内使用 → `src/views/<view>/components/<name>/` 或对应功能目录下

涉及页面时，常见约定：

- 页面/组件样式默认使用 `<style scoped lang="scss">`
- 样式需要跨组件复用时再使用 `.module.scss`
- `types/` 下为文件夹，含 `model.ts`、`api.ts`
...
- **已提供接口**：按 `.agents/rules/03-项目结构.md` 等规范，在 tasks 中安排 `types/`、`services/` 等。
- **未就绪**：按项目 Mock 数据策略，在 `types/<feature>/` 下定义 `model.ts`、`api.ts`，在 `services/` 下提供 mock；tasks 中标注「mock，后续替换」。
...
- **新页面**：路由目录、`index.vue`、默认 `<style scoped lang="scss">`（按需再拆 `index.module.scss`）、与布局一致的结构；若有分析清单则写「依据 xxx-UI分析清单 实现」。
- **功能组件**：组件目录、`index.vue`（按需再拆 `index.module.scss`）、占位与规范。
- **接口/数据层**：`types/<feature>/`、`services/<feature>.ts` 或 mock。
