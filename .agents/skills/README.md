...
- `create-route`：创建与维护路由目录与页面组件（配合 `06-路由规范` 使用）
- `create-store`：使用 Pinia 创建与维护全局状态（配合 `07-状态管理` 使用）
- `theme-variables`：正确使用 Ant Design Vue 与自定义主题 CSS 变量（配合 `09-样式规范` 使用）
...
- `vercel-react-best-practices`：React/Next.js 性能优化实践（第三方参考）
- `vercel-composition-patterns`：组件组合模式实践（第三方参考）
